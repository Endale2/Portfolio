...
const FONTS = {
  code: 'font-["JetBrains Mono","Fira Code","Monaco","Cascadia Code","Roboto Mono",Consolas,"Courier New",monospace]',
  sans: 'font-sans',
};
...
const projectsData = [
  {
    id: 'n8n-autonewsbot', // ✨ Added unique ID for stable key
...
// ✨ New component just for rendering links (more maintainable)
const ProjectLinks = ({ project }) => (
  <div className="flex flex-wrap gap-1.5 mt-1">
    {Object.keys(LINK_ICONS)
      .filter((key) => project[key]) // Only show links that exist on the project object
      .map((key) => (
        <a
          key={key}
          href={project[key]}
          ...
          className={`flex items-center gap-1 text-cyan-300 bg-slate-800/80 hover:bg-cyan-700/90 transition-all duration-300 rounded-md px-2 py-0.5 shadow text-[11px] font-medium hover:scale-105 ${FONTS.code}`}
        >
          {LINK_ICONS[key]}
...
// ✨ New component for the status badge
const StatusBadge = ({ status }) => {
  const config = STATUS_CONFIG[status];
  return (
    <span
      className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${config.badge} ${FONTS.code}`}
    >
      {config.label}
    </span>
  );
...
// ✨ New component for the card's image section
const ProjectImage = ({ title, coverImage, category, borderStyle }) => (
  <div className="md:w-1/2 w-full flex justify-center items-center relative">
    <img
      src={coverImage}
      alt={title}
      className={`w-full max-w-xs h-28 md:h-36 object-cover rounded-lg border-2 ${borderStyle} shadow-md transition-transform duration-500`}
    />
    <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-black/80 via-black/50 to-transparent rounded-b-lg p-2">
      <h3
        className={`text-sm md:text-base font-bold text-white truncate ${FONTS.code}`}
      >
        {title}
      </h3>
      <span className={`text-cyan-300 text-[11px] ${FONTS.code}`}>
        {category}
      </span>
    </div>
  </div>
...
// ✨ Main Project Card, now composed of smaller components
  ...
  return (
    ...
    >
      ...
      <div className="md:w-1/2 w-full flex flex-col justify-center gap-2">
        ...
        <p className={`text-slate-200 text-xs leading-relaxed ${FONTS.sans}`}>
          {description}
        </p>
...
const SectionHeader = () => (
  <>
    <div className={`mb-2 text-cyan-400/70 text-xs pl-1 ${FONTS.code}`}>
      {'// Featured Projects'}
    </div>
    <div className="flex items-center mb-10">
      <span
        className={`text-cyan-400 ${FONTS.code} text-lg mr-3 bg-slate-800/70 px-2 py-0.5 rounded shadow border border-cyan-400/30`}
      >
        03.
      </span>
      <h2
        className={`text-xl md:text-2xl font-extrabold bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent tracking-tight ${FONTS.code}`}
      >
        ...
        <div className="relative">
          ...
          <div className="flex flex-col gap-10 md:gap-16">
            {projectsData.map((project, index) => (
              <div key={project.id} className="relative flex justify-center"> {/* ✨ Using ID as key */}

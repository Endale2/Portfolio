...
const Contact = () => {
  ...
  return (
    <section
      id="contact"
      className="relative py-20 bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 text-white overflow-hidden"
    >
      {/* Decorative background lights */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-10 left-1/4 w-2/3 h-64 bg-cyan-500/10 blur-3xl rounded-full animate-pulse" />
        <div className="absolute bottom-10 right-1/4 w-1/2 h-56 bg-blue-500/10 blur-2xl rounded-full animate-pulse" />
      </div>
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4">
        ...
        <motion.div
          ...
          className="flex items-center mb-10 md:mb-14"
        >
          <span
        ...
        <motion.p
          ...
          whileInView="visible"
          viewport={{ once: true, amount: 0.3 }}
          className={`text-base md:text-lg text-slate-300 mb-10 leading-relaxed max-w-2xl mx-auto text-center ${sansFont}`}
        >
          I’m open to new opportunities, collaborations, or just a friendly chat. 
          Drop me a message or connect via social media!
        </motion.p>
        ...
        >
          {socialLinks.map(({ icon: Icon, url, label }, idx) => (
            <a
              ...
              aria-label={label}
              className={`text-slate-400 hover:text-cyan-400 transition-all duration-300 transform hover:scale-110 hover:rotate-6 p-3 rounded-full bg-slate-800/60 hover:bg-slate-800/90 border border-slate-700 hover:border-cyan-400/60 focus:outline-none focus:ring-2 focus:ring-cyan-500 ${codeFont}`}
            >
        ...
        <motion.form
          ...
          className="glass rounded-2xl border border-cyan-400/20 p-6 md:p-10 shadow-2xl backdrop-blur-md bg-slate-900/60"
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
            <div>
              <label
                htmlFor="name"
                className={`block text-sm font-semibold text-slate-300 mb-2 ${codeFont}`}
              >
                Name *
              </label>
              <input
                ...
                required
                className={`w-full p-3 bg-slate-800/60 border border-slate-700 rounded-lg focus:ring-2 focus:ring-cyan-500 outline-none ${sansFont} text-slate-200 placeholder-slate-400`}
              />
            </div>
            <div>
              <label
          ...
          <div className="mb-8">
            ...
            <textarea
              ...
              onChange={handleChange}
              required
              className={`w-full p-3 bg-slate-800/60 border border-slate-700 rounded-lg focus:ring-2 focus:ring-cyan-500 outline-none resize-none ${sansFont} text-slate-200 placeholder-slate-400`}
            />
          </div>
          <motion.button
            type="submit"
